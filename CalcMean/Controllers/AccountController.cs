...
namespace CalcMean.Controllers
{
    public class AccountController : BaseController
    {
        public AccountController()
            : this(new UserManager<CmUser>(new UserStore<CmUser>(new CmContext())))
        {
        }

        public AccountController(UserManager<CmUser> userManager)
        {
            UserManager = userManager;
        }

        public UserManager<CmUser> UserManager { get; private set; }
        ...
        public async Task<ActionResult> Register(RegisterViewModel model)
        {
            if (ModelState.IsValid)
            {
                var user = new CmUser() { UserName = model.UserName, Name = model.Name, PhoneNumber = model.Phone, CreateDate = DateTime.Now };
                var result = await UserManager.CreateAsync(user, model.Password);
                if (result.Succeeded)
                {
                    await SignInAsync(user, isPersistent: false);
                    return RedirectToAction("Index", "DsChiTieu");
                }
                else
                {
                    AddErrors(result);
        ...
        public async Task<ActionResult> ExternalLoginConfirmation(ExternalLoginConfirmationViewModel model, string returnUrl)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                var user = new CmUser() { UserName = model.UserName };
        ...
        private async Task SignInAsync(CmUser user, bool isPersistent)
        {
